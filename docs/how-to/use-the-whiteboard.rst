...
Each one is a single step you can undo. The whiteboard's own toolbar keeps to the board itself: zoom, the buttons that put something on the board, and the **Structure** and **Parking lot** toggles that show and hide its two side panels. The ribbon's **Whiteboard** tab toggles the same panels from its **Show** group. Both the host and everyone who joins a planning session get these toggles on the board's toolbar.
...
- **Right-click a row** (or focus it and press :kbd:`Shift+F10`) for that task's menu: **Rename**, **Edit task…** (the task details form), **Assign resources…**, **Indent** (make it a subtask of the task above it), **Outdent** (move it up a level, to just after its current parent) and **Delete task**, which asks first and removes its subtasks and note with it. Indent and Outdent are greyed out when the task has nowhere to go. Each change is a single undo step, and the menu works the same for joiners in a planning session.
...
Hide the panel with the ``‹`` button in its header, or **Structure** on the board's toolbar (or the ribbon's **Whiteboard** tab); a small **Structure** tab stays pinned to the canvas edge to bring it back. Whether the panel is open, and which branches you collapsed, is remembered per project on this device — it is never written into your plan file.
