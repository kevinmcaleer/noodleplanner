...
- **Adopting the warm palette wholesale**, which is now the only substantial
  item left and the reason `raw-colour` still stands at 751.
  ...
  **The surface and text tokens invert between themes.** `--np-paper` is
  ...
  So the work is: classify each of the 751 as *follows the theme* or *fixed
  ...
  **✅ The decision is taken.** "576 carry a hue, so the colour is the meaning"
  ...
  `raw-colour` went **1,057 → 751**. What is left is the genuinely semantic
...
is named, gated and adopted, and the count fell 1,057 → 751. What remains is
