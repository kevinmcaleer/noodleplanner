...
| `no-ascii ms` | The same call without the `ascii_output` string that `/api/parse` callers never read |
...
prints an identifying line for exactly that reason. Numbers measured on a
