...
`scripts/benchmark_parse.py` measures how much server CPU a plan parse costs.
The frontend calls `POST /api/parse` on essentially every plan change, so this
is the hot path for the backend — and it matters most on the Raspberry Pi that
hosts the app, where CPU is the scarce resource (see issue #788).

```bash
# Default sizes: 110, 275, 550 and 1,100 tasks
uv run scripts/benchmark_parse.py

# Custom sizes, in blocks of 11 leaf tasks
uv run scripts/benchmark_parse.py --repeats 10 100

# Markdown table, for pasting into an issue or PR
uv run scripts/benchmark_parse.py --markdown

# Parse timings only, skipping the full-endpoint pass
uv run scripts/benchmark_parse.py --no-http
```

Three columns are reported per plan size:

| Column | What it measures |
|---|---|
| `parse ms` | `PlanService.parse()` as `/api/parse` runs it: the plan scheduled once, no ASCII table (Phase 0, issue #789) |
| `with-ascii ms` | The same call with `include_ascii=True`, the pre-#789 behaviour that scheduled the plan a second time to build a string only `/render` uses; `saving` is how much of that the default path no longer pays |
| `HTTP ms` | The full endpoint, adding request validation and JSON encoding |

Plans are generated by repeating a fixed block of 11 leaf tasks across 6
phases, with names suffixed per copy so `[depends ...]` references stay
unambiguous. Timings are medians, with a warm-up run discarded.

Results are machine-specific, so quote the hardware alongside them — the script
prints an identifying line for exactly that reason. The before/after tables for
Phase 0 are in `tests/benchmarks/parse_benchmark.md`; numbers measured on the
Raspberry Pi 5 are recorded on issue #788.
