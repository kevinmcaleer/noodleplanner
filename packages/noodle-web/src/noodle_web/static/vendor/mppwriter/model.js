...
export const REL_TYPES = { FF: 0, FS: 1, SF: 2, SS: 3 };
/**
...
export const BASELINE_UNSET_DOUBLE = Uint8Array.from("8dedb5a0f7c6b0be".match(/../g).map((h) => parseInt(h, 16)));
/**
...
export const NULL_RESOURCE_GUID = Uint8Array.from("788bcba08c2a6d4300000000000000ff".match(/../g).map((h) => parseInt(h, 16)));
...
export const UNITS_CODES = { m: 3, h: 5, d: 7, w: 9, mo: 11 };
...
export const WORK_WINDOWS = [[8 * 60, 12 * 60], [13 * 60, 17 * 60]];
...
const weekdayOf = (d) => (d.getUTCDay() + 6) % 7;
const minutesOf = (d) => d.getUTCHours() * 60 + d.getUTCMinutes();
const atMinutes = (d, minutes) => new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(), Math.floor(minutes / 60), minutes % 60));
export function workPattern(cal) {
    const windows = new Map();
    for (let wd = 0; wd < 5; wd++)
        windows.set(wd, WORK_WINDOWS.map((w) => [...w]));
...
const DEFAULT_PATTERN = () => workPattern(null);
/** Working time between two datetimes, in tenths of a minute. */
export function workingTenths(start, finish, pattern) {
...
export function advanceWorking(start, tenths, pattern) {
    const { windows, nonworking } = pattern ?? DEFAULT_PATTERN();
    ...
    for (let i = 0; i < 36600; i++) {
        ...
        if (!nonworking.has(key)) {
            for (const [w0, w1] of windows.get(weekdayOf(day)) ?? []) {
                ...
                if (w1 > lo) {
                    if (minutes <= w1 - lo)
                        return atMinutes(day, lo + minutes);
                    minutes -= w1 - lo;
                }
            }
        }
 ...
 */
export function nextWorkingMoment(point, pattern) {
    const { windows, nonworking } = pattern ?? DEFAULT_PATTERN();
 ...
 */
export function previousWorkingMoment(point, pattern) {
    const { windows, nonworking } = pattern ?? DEFAULT_PATTERN();
    let day = new Date(Date.UTC(point.getUTCFullYear(), point.getUTCMonth(), point.getUTCDate()));
    const minute = minutesOf(point);
    const pointKey = dayKey(point);
    for (let i = 0; i < 3700; i++) {
        ...
        if (!nonworking.has(key)) {
            ...
            const ends = (windows.get(weekdayOf(day)) ?? [])
                ...
                .map(([, w1]) => w1);
            if (ends.length)
                return atMinutes(day, Math.max(...ends));
        }
        day = new Date(day.getTime() - DAY_MS);
    }
    return null;
...
export function weeklyOverlapMinutes(a, b) {
    let total = 0;
 ...
 */
export function linkDrivenStart(rel, predEff, succDurTenths, pattern) {
    ...
    if (type === "FS") {
        ...
        if (lag === 0 && succDurTenths === 0)
            return predEff.finish;
 ...
 * will describe.
 */
export function effectiveSchedule(project) {
 ...
 * save date in Props.
 */
export function setBaseline(project, slot = 0) {
    ...
    for (const asn of assignments) {
        ...
        if (!e)
            continue;
        directWork.set(asn.taskUid, (directWork.get(asn.taskUid) ?? 0) + e.tenths * WORK_SCALE * (asn.units ?? 1));
    }
    const wsum = new Map();
    for (const t of deepestFirst) {
        const kids = children.get(t.uid) ?? [];
 ...
 */
export function clearBaseline(project, slot = 0) {
    if (!Number.isInteger(slot) || slot < 0 || slot > 10) {
        throw new Error(`baseline slot must be 0-10 (got ${slot})`);
    }
    ...
    for (const h of holders) {
        if (h.baselines)
            delete h.baselines[slot];
    }
}
/** The slots a project has saved, on any entity, in order. */
export function baselineSlots(project) {
    const slots = new Set();
    for (const t of project.tasks)
        for (const s of Object.keys(t.baselines ?? {}))
            slots.add(Number(s));
    return [...slots].sort((a, b) => a - b);
}
// ------------------------------------------------------------------ notes --
/** Wrap plain text in the minimal RTF envelope Project writes for notes. */
export function encodeRtfNotes(text) {
    let body = "";
...
export function decodeRtfNotes(rtf) {
    const text = String.fromCharCode(...rtf);
    let body = text.split("\\fs20 ").pop() ?? "";
    if (body.endsWith("}"))
        body = body.slice(0, -1); // the group terminator, not an escape
    let out = "";
    let i = 0;
    while (i < body.length) {
        const ch = body[i];
        if (ch !== "\\") {
            ...
            let j = i + 2;
            let digits = "";
            while (j < body.length && (/\d/.test(body[j]) || (body[j] === "-" && !digits)))
                digits += body[j++];
...
export function validate(project) {
    ...
    for (const t of project.tasks) {
        if (t.uid <= 0)
            throw new Error(`task uid must be > 0 (got ${t.uid})`);
        if (seen.has(t.uid))
            throw new Error(`duplicate task uid ${t.uid}`);
        seen.add(t.uid);
        if (t.finish.getTime() < t.start.getTime())
            throw new Error(`task ${t.uid}: finish before start`);
    }
    ...
    for (const t of project.tasks) {
        ...
        if (level < 1)
            throw new Error(`task ${t.uid}: outlineLevel must be >= 1`);
        while (stack.length && (stack[stack.length - 1].outlineLevel ?? 1) >= level)
            stack.pop();
        const top = stack[stack.length - 1];
        const expectedParent = level > 1 && top ? top.uid : 0;
        if (level > 1 && (!top || (top.outlineLevel ?? 1) !== level - 1)) {
            throw new Error(`task ${t.uid}: outlineLevel ${level} does not follow a level ${level - 1} task`);
        }
        if ((t.parentUid ?? 0) !== expectedParent) {
            throw new Error(`task ${t.uid}: parentUid ${t.parentUid ?? 0} does not match the outline ` +
    ...
    for (const rel of project.relations ?? []) {
        for (const end of [rel.predUid, rel.succUid]) {
            if (!seen.has(end))
                throw new Error(`relation references unknown task uid ${end}`);
        }
        if (rel.predUid === rel.succUid)
            throw new Error(`task ${rel.predUid} cannot depend on itself`);
    ...
    for (const root of succs.keys()) {
        ...
        state.set(root, 1);
        const trail = [root];
        const stackIt = [[root, succs.get(root) ?? [], 0]];
