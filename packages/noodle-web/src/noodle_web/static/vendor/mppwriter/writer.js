...
function dayBlocks(cal) {
    const out = [];
    for (let block = 0; block < 7; block++) {
        const wd = (block + 6) % 7; // block 0 = Sunday = weekday 6
        const val = cal.week?.[wd];
        if (val === undefined)
            out.push([B.CAL_DAY_DEFAULT, []]);
        else if (val === null)
            out.push([B.CAL_DAY_NONWORKING, []]);
        else
            out.push([B.CAL_DAY_WORKING, val]);
    }
    return out;
}
function exceptionTuples(cal) {
    return (cal.exceptions ?? [])
        .map((x) => [x.start, x.finish ?? x.start, x.name ?? ""])
...
export class MppWriter {
    ...
    calUidCol = 0;
    calStandardUid = null;
    calStandardGuid = new Uint8Array(16);
    calMetaHdr;
    calMeta2Hdr;
    calVarHdr;
    calVarEntries = [];
    calVarHi = B.RESOURCE_FIELD_HI;
    ...
    get(path) {
        ...
        if (!data)
            throw new Error(`template has no stream ${path}`);
        return data;
    }
    has(path) {
    ...
    meta(path, defaultSize) {
        return B.parseFixedMetaAuto(this.get(path), defaultSize);
    }
    /** Write a value into whichever block the class's field map puts it in. */
    putf(fm, native, rec, rec2, name, kind, value) {
        const it = fm.get(native[name]);
        if (!it)
            return;
        const dst = it.block === 0 ? rec : rec2;
        const size = kind === "f64" ? 8 : kind === "u16" ? 2 : 4;
        if (it.offset + size > dst.length)
            return;
        const d = dv(dst);
        if (kind === "u32")
            d.setUint32(it.offset, value >>> 0, true);
        else if (kind === "i32")
            d.setInt32(it.offset, value | 0, true);
        else if (kind === "u16")
            d.setUint16(it.offset, value & 0xffff, true);
        else
            d.setFloat64(it.offset, value, true);
    }
    putfBytes(fm, native, rec, rec2, name, value) {
        const it = fm.get(native[name]);
        if (!it)
            return;
        const dst = it.block === 0 ? rec : rec2;
        if (it.offset + value.length <= dst.length)
            dst.set(value, it.offset);
    }
    putfTs(fm, native, rec, rec2, name, date) {
        this.putfBytes(fm, native, rec, rec2, name, B.encodeTimestamp(date));
    }
    bitf(bits, native, meta, meta2, name, value) {
        const idx = bits.get(native[name]);
        if (idx !== undefined)
            B.setMetaBit(meta, meta2, idx, value);
    }
    put(rec, name, kind, value) {
        const it = this.taskFm.get(NATIVE[name]);
        if (!it || it.block !== 0)
            return;
        const size = kind === "f64" ? 8 : kind === "u16" ? 2 : 4;
        if (it.offset + size > rec.length)
            return;
        const d = dv(rec);
        if (kind === "u32")
            d.setUint32(it.offset, value >>> 0, true);
    ...
    putTs(rec, name, date) {
        const it = this.taskFm.get(NATIVE[name]);
        if (it && it.block === 0)
            rec.set(B.encodeTimestamp(date), it.offset);
    }
    putBit(meta, meta2, name, value) {
    ...
    loadPrototypes() {
        ...
        const full = recs.map((r, i) => [i, r]).filter(([, r]) => r.length > 100).map(([i]) => i);
        if (!full.length)
            throw new Error("template has no task records to use as prototypes");
        const summaryI = full[0];
        const sumBit = this.taskBit.get(NATIVE["SUMMARY"]);
        const taskI = full.slice(1).find((i) => sumBit === undefined || !B.metaBit(m.items[i], m2.items[i], sumBit)) ??
            (full.length > 1 ? full[full.length - 1] : full[0]);
        const protoFor = (i) => {
            const uid = dv(recs[i]).getUint32(0, true);
            const byType = vm.table.get(uid) ?? new Map();
            const vars = [...byType.entries()]
                .sort((a, b) => a[0] - b[0])
                .map(([typ, off]) => [typ, B.readVar(vdata, off)]);
        ...
        for (let i = 0; i < arecs.length; i++) {
            ...
            if (rec.length > 50 && i < arecs2.length) {
                ...
                if (taskIt && dv(rec).getUint32(taskIt.offset, true) === 0)
                    continue;
                const uid = dv(rec).getUint32(0, true);
                const byType = avm.table.get(uid) ?? new Map();
                this.assnProto = {
                    rec,
                    rec2: arecs2[i],
                    meta: am.items[i],
        ...
        rsrecs.forEach((rec, i) => {
            ...
            const byType = rec.length > 16 ? (rsvm.table.get(uid) ?? new Map()) : new Map();
            const row = {
                rec,
        ...
        if (this.rscProto) {
            ...
            if (it) {
                ...
                if (it.offset + 4 <= src.length)
                    rsc0CalUid = dv(src).getInt32(it.offset, true);
            }
        }
        for (const row of this.calRows) {
            if (row.rec.length !== 12 || !rsc0CalUid)
                continue;
    ...
    build(project) {
        ...
        const directWork = new Map();
        for (const asn of assignments) {
            const tenths = eff.get(asn.taskUid).tenths;
            directWork.set(asn.taskUid, (directWork.get(asn.taskUid) ?? 0) + tenths * WORK_SCALE * (asn.units ?? 1));
        }
        const wsum = new Map();
        ...
        for (const t of deepestFirst) {
            ...
            if (pct < 0 || pct > 100)
                throw new Error(`task ${t.uid}: percentComplete out of range`);
            for (const [label, rec, limit] of [
                ...
                for (const n of Object.keys(rec ?? {}).map(Number)) {
                    if (!(n >= 1 && n <= limit))
                        throw new Error(`task ${t.uid}: ${label}${n} out of range 1..${limit}`);
                }
            }
            const kids = children.get(t.uid) ?? [];
            if (kids.length) {
                const tot = kids.reduce((s, k) => s + eff.get(k.uid).tenths, 0);
                pctEff.set(t.uid, tot ? Math.round(kids.reduce((s, k) => s + eff.get(k.uid).tenths * pctEff.get(k.uid), 0) / tot) : 0);
            }
            else {
        ...
        for (const rel of relations) {
            ...
            if (!prev || s.getTime() > prev.getTime())
                linkStart.set(rel.succUid, s);
        }
        const warn = (message) => this.onWarning(new ScheduleWarning(message));
        ...
        for (const t of tasks) {
            ...
            if (cal && assignedTasks.has(t.uid) && !weeklyOverlapMinutes(workPattern(cal), pattern)) {
                warn(`task ${t.uid} ${JSON.stringify(t.name)} is on calendar ${JSON.stringify(cal.name ?? "")}, ` +
        ...
        for (const t of tasks) {
            if (t.manual || children.get(t.uid)?.length || eff.get(t.uid).tenths === 0)
                continue;
            ...
            if (rolled.getTime() !== eff.get(t.uid).start.getTime()) {
                warn(`task ${t.uid} ${JSON.stringify(t.name)} starts ${fmtDate(eff.get(t.uid).start)}, which is ` +
        ...
        for (const [uid, implied] of linkStart) {
            ...
            if (task.manual || children.get(uid)?.length || task.calendar)
                continue;
        ...
        const calRowsOut = [...this.calRows];
        let calMetaPatched = false;
        const calVarNew = [];
        const namedCalUid = new Map([["Standard", stdUid]]);
        ...
        for (const cal of project.calendars ?? []) {
            if (!this.calBaseRow)
                throw new Error("template has no base calendar record to clone");
            const name = cal.name ?? "Standard";
            if (namedCalUid.has(name))
                throw new Error(`duplicate calendar name ${JSON.stringify(name)}`);
            const uid = nextCalUid++;
            namedCalUid.set(name, uid);
            const crec = new Uint8Array(12);
            const cd = dv(crec);
            for (let j = 0; j < 3; j++)
                cd.setInt32(j * 4, j === this.calUidCol ? uid : -1, true);
        ...
        const emit = (proto, uid, tid, name, start, finish, durTenths, level, parentUid, guid, parentGuid, isSummary, position, units = "d", estimated = false, work = 0, calUid = null, task = null, pct = 0, baselines = {}) => {
            ...
            for (const f of ["START", "EARLY_START", "LATE_START"])
                this.putTs(rec, f, start);
            for (const f of ["FINISH", "EARLY_FINISH", "LATE_FINISH"])
                this.putTs(rec, f, finish);
            ...
            if (pct) {
                ...
                if (pct === 100)
                    this.putTs(rec, "ACTUAL_FINISH", finish);
                this.putTs(rec, "STOP", point);
                this.putTs(rec, "RESUME", point);
                for (const f of ["PERCENT_COMPLETE", "ACTUAL_START", "ACTUAL_DURATION"])
                    this.putBit(m, m2, f, true);
            }
            const extraVars = [];
            if (task) {
                if (task.notes)
                    extraVars.push([NATIVE["NOTES"], encodeRtfNotes(task.notes)]);
                if (task.wbs !== undefined)
                    extraVars.push([NATIVE["WBS"], B.encodeUnicode(task.wbs)]);
        ...
        if (relations.length && !this.relProto) {
            throw new Error("template has no dependency records to use as a prototype; save the template with at " +
        ...
        if (resources.length) {
            if (!this.rscProto)
                throw new Error("template has no uid-0 resource record to use as a prototype");
            ...
            const rrows = [...this.rscRows];
            const rvarEntries = [];
            for (const row of this.rscRows) {
                if (row.rec.length > 16) {
                    const uid = dv(row.rec).getUint32(0, true);
                    for (const [typ, payload] of row.var ?? [])
                        rvarEntries.push({ uid, type: typ, payload });
                }
            }
        ...
        const rscGuid = new Map();
        for (const res of resources)
            rscGuid.set(res.uid, res.guid ?? new Uint8Array(16));
        for (const asn of assignments) {
        ...
        specs.forEach(([taskUid, resourceUid, units, specBaselines], i0) => {
            ...
            const put = (name, kind, value) => this.putf(this.assnFm, ASSN_NATIVE, rec, rec2, name, kind, value);
            put("UNIQUE_ID", "u32", i);
            ...
            for (const name of ["WORK", "REGULAR_WORK", "REMAINING_WORK"])
                put(name, "f64", work);
            ...
            this.putfBytes(this.assnFm, ASSN_NATIVE, rec, rec2, "RESOURCE_GUID", empty ? NULL_RESOURCE_GUID : (rscGuid.get(resourceUid) ?? new Uint8Array(16)));
        ...
        for (const [storage, key] of Object.entries(B.PROPS_VAR2DATA_SIZE)) {
            if (this.props.has(key)) {
                ...
                if (data)
                    this.props.set(key, u32(data.length));
            }
        }
        if (project.defaultCalendar !== undefined) {
            ...
            if (this.props.has(B.PROPS_DEFAULT_CALENDAR_NAME)) {
                this.props.set(B.PROPS_DEFAULT_CALENDAR_NAME, concat([B.encodeUtf16le(project.defaultCalendar), new Uint8Array(4)]));
            }
        }
...
export function writeProject(template, project, opts) {
