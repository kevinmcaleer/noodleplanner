...
export class Storage {
    ...
    addStorage(name) {
        ...
        if (existing instanceof Storage)
            return existing;
        const s = new Storage();
...
export function writeCfb(root, rootClsid = new Uint8Array(16)) {
    const entries = [];
    const rootEntry = new Entry("Root Entry", TYPE_ROOT);
    rootEntry.clsid = rootClsid;
    entries.push(rootEntry);
    const flatten = (storage, parent) => {
        const kids = [];
        for (const [name, val] of storage.children) {
            const e = val instanceof Storage ? new Entry(name, TYPE_STORAGE) : new Entry(name, TYPE_STREAM, val);
            entries.push(e);
        ...
        for (const e of kids) {
            if (e.etype === TYPE_STORAGE)
                flatten(storage.children.get(e.name), e);
        }
    };
    rootEntry.index = 0;
    ...
    const dirEntries = entries.map((e) => dirEntry(e));
    const slotBytes = pad(new Uint8Array(dirEntries.length * 128), SECTOR).length;
    while (dirEntries.length * 128 < slotBytes)
        dirEntries.push(dirEntry(null));
    ...
    for (;;) {
        ...
        if (neededFat === nFat && neededDifat === nDifat)
            break;
        nFat = neededFat;
        nDifat = neededDifat;
    }
    ...
    for (let i = 0; i < nFat; i++)
        fatFull.push(FATSECT);
    for (let i = 0; i < nDifat; i++)
        fatFull.push(DIFSECT);
    while (fatFull.length < nFat * FAT_PER_SECTOR)
        fatFull.push(FREESECT);
    ...
    for (let i = 0; i < nFat; i++)
        sectors.push(fatRaw.subarray(i * SECTOR, (i + 1) * SECTOR));
    for (let i = 0; i < nDifat; i++) {
        ...
        for (let j = 0; j < DIFAT_PER_SECTOR; j++)
            view.setUint32(j * 4, chunk[j] ?? FREESECT, true);
    ...
    for (let i = 0; i < 109; i++)
        hv.setUint32(76 + i * 4, fatSectorIds[i] ?? FREESECT, true);
...
export function readCfb(bytes) {
    ...
    for (let i = 0; i < MAGIC.length; i++) {
        if (bytes[i] !== MAGIC[i])
            throw new Error("not a compound file (bad magic)");
    }
    ...
    const readSector = (id) => bytes.subarray(sectorOffset(id), sectorOffset(id) + sectorSize);
    ...
    if (minifatStart < FATSECT) {
        ...
        for (let i = 0; i < mf.length / 4; i++)
            minifat.push(mv.getUint32(i * 4, true));
    }
    ...
    const walk = (index, into) => {
        if (index === NOSTREAM || index >= raw.length)
            return;
        const e = raw[index];
        walk(e.left, into);
        if (e.etype === TYPE_STORAGE) {
