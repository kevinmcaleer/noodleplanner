...
const BEN_COLOURS = {
    ...
    change:     { fill: '#EBEBEB', text: '#1A1A1A', stroke: 'none' },
...
function updateBenefits() {
    ...
    // Attach keyboard handler to benefits container (once)
    const container = document.getElementById('benefitsContainer');
    if (container && !container._benKeydownAttached) {
        container.addEventListener('keydown', (e) => {
            // Skip if focused on an input inside the form
...
/**
 * Copy the benefits SVG canvas as a PNG image to the clipboard.
 */
async function copyBenefitsAsImage() {
    if (!benSvg) {
        alert('No benefits map to copy.');
        return;
    }
    ...
    try {
        ...
        // Set explicit dimensions and background on clone
...
/**
 * Export benefit items to an Excel spreadsheet via the backend API.
 */
async function exportBenefitsExcel() {
    if (benefitItems.length === 0) {
        alert('No benefit items to export. Add some items first.');
        return;
    }

    try {
        const projectName = document.getElementById('projectName')
            ? document.getElementById('projectName').textContent.trim()
            : 'Benefits';
