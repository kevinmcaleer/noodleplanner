 ...
 * msproject-task-diff.js -- this tolerates plans that mix 2-space and
 * 4-space indent steps, which a fixed indentDiff check does not.
 */
function getTaskDescendants(parentLineNumber, lines) {
    ...
    const descendants = [];
    const indentStack = [parentIndent];
    for (let i = parentLineNumber; i < lines.length; i++) {
        ...
        // If we hit a line at same or lower indentation than the parent, its subtree is over
        ...
        const depth = indentStack.length; // 1 = direct child, 2 = grandchild, ...
