...
function setupEditor(editor, lineNumbers, highlightLayer, shouldRender) {
    ...
    editor.addEventListener('keydown', function(e) {
        ...
        // Find with Cmd+F or Ctrl+F - open custom find dialog
        ...
        // Find and Replace with Cmd+H or Ctrl+H
...
function initializeGlobalKeyboardShortcuts() {
    document.addEventListener('keydown', function(e) {
        ...
        // Find Next with F3 (only if find dialog is open)
        if (e.key === 'F3' && !isInputField) {
            e.preventDefault();
...
<ul>
    ...
    <li><strong>Cmd/Ctrl + H</strong> - Find and replace text</li>
</ul>
...
<ul>
    ...
    <li><strong>Enter</strong> - Find next (in find field) / Replace current (in replace field)</li>
    <li><strong>Ctrl/Cmd + Enter</strong> - Replace all (in replace field)</li>
    <li><strong>Escape</strong> - Close find & replace dialog</li>
</ul>
...
function openFindReplace(mode = 'find') {
    ...
    if (activeElement && activeElement.id === 'planEditor') {
        editor = activeElement;
    } else if (activeElement && activeElement.id === 'kanbanPlanEditor') {
        editor = activeElement;
    } else {
    ...
    if (!editor) {
        console.error('No editor found for find-and-replace');
        return;
    }

    findReplaceState.currentEditor = editor;
    findReplaceState.mode = mode;
    findReplaceState.isOpen = true;

    // Show the modal
    const overlay = document.getElementById('findReplaceOverlay');
    const title = document.getElementById('findReplaceTitle');

    if (mode === 'replace') {
        title.textContent = 'Find & Replace';
        document.getElementById('replaceInput').closest('.form-group').style.display = 'block';
    } else {
        title.textContent = 'Find';
        document.getElementById('replaceInput').closest('.form-group').style.display = 'none';
    }

    overlay.classList.add('active');

    // Focus on the find input and select any existing text
    ...
    // If there's selected text in the editor, use it as the initial search term
    if (editor.selectionStart !== editor.selectionEnd) {
        const selectedText = editor.value.substring(editor.selectionStart, editor.selectionEnd);
        if (selectedText && selectedText.length < 100) { // Reasonable length limit
...
function closeFindReplace() {
    ...
    if (findReplaceState.currentEditor) {
        findReplaceState.currentEditor.focus();
    }
...
/**
 * Check if the find-and-replace dialog is open
 */
...
function performFind() {
    ...
    if (!query || !findReplaceState.currentEditor) {
        clearFindHighlights();
        updateFindResults();
        return;
    }

    findReplaceState.lastQuery = query;
    ...
    try {
        findMatches(query, caseSensitive, useRegex, wholeWords);
        highlightMatches();
        updateFindResults();

        // Jump to first match if any exist
        if (findReplaceState.matches.length > 0) {
            findReplaceState.currentMatch = 0;
            scrollToCurrentMatch();
        }
    } catch (error) {
        console.error('Find error:', error);
        updateFindResults(error.message);
    }
}
 ...
 */
function findMatches(query, caseSensitive, useRegex, wholeWords) {
    const editor = findReplaceState.currentEditor;
    const text = editor.value;
    findReplaceState.matches = [];
...
function highlightMatches() {
    // For now, we'll use a simple approach by managing selection
    // In a more advanced implementation, we could create an overlay
...
function clearFindHighlights() {
    // Clear any visual highlights here
    // For now, just clear the selection
    if (findReplaceState.currentEditor) {
        // Don't clear selection as it might interfere with user interaction
    }
}

/**
 * Update the current match selection in the editor
 */
function updateCurrentMatchSelection() {
    if (!findReplaceState.currentEditor || findReplaceState.matches.length === 0 || findReplaceState.currentMatch < 0) {
        return;
    }

    const match = findReplaceState.matches[findReplaceState.currentMatch];
    if (match) {
        findReplaceState.currentEditor.setSelectionRange(match.start, match.end);
        findReplaceState.currentEditor.focus();
    }
}
...
function scrollToCurrentMatch() {
    ...
    // Scroll the textarea to show the current match
    const editor = findReplaceState.currentEditor;
    const match = findReplaceState.matches[findReplaceState.currentMatch];

    // Calculate approximate line number for scrolling
 ...
 */
function updateFindResults(errorMessage = null) {
    const resultsText = document.getElementById('findResultsText');

    if (errorMessage) {
        resultsText.textContent = `Error: ${errorMessage}`;
        resultsText.style.color = '#dc3545';
        return;
    }
    ...
    if (findReplaceState.matches.length === 0) {
        resultsText.textContent = findReplaceState.lastQuery ? 'No results' : 'Enter search terms';
    } else {
        const current = findReplaceState.currentMatch + 1;
        const total = findReplaceState.matches.length;
        resultsText.textContent = `${current} of ${total}`;
    }
}
...
function findNext() {
    if (findReplaceState.matches.length === 0) {
        performFind(); // Try to find matches first
        return;
    }
...
function findPrevious() {
    ...
    findReplaceState.currentMatch = findReplaceState.currentMatch <= 0
        ? findReplaceState.matches.length - 1
        : findReplaceState.currentMatch - 1;
    scrollToCurrentMatch();
    updateFindResults();
}
 ...
 */
function replaceOne() {
    if (findReplaceState.matches.length === 0 || findReplaceState.currentMatch < 0) {
        return;
    }
    ...
    editor.setSelectionRange(match.start + replacement.length, match.start + replacement.length);

    // Refresh the search (this will update match positions)
    performFind();

    // If we still have matches, stay on the same logical position or move to next
    if (findReplaceState.matches.length > 0) {
        // Move to next match or stay at the same index if it's the last one
        if (findReplaceState.currentMatch >= findReplaceState.matches.length) {
            findReplaceState.currentMatch = findReplaceState.matches.length - 1;
        }
        scrollToCurrentMatch();
    }
...
function replaceAll() {
    ...
    const totalReplaced = findReplaceState.matches.length;

    // Replace from end to beginning to preserve indices
    const matches = [...findReplaceState.matches].reverse();

    let newValue = editor.value;
    for (const match of matches) {
        const before = newValue.substring(0, match.start);
        const after = newValue.substring(match.end);
    ...
    // Clear matches and update display
    findReplaceState.matches = [];
    findReplaceState.currentMatch = -1;

    // Show success message
    document.getElementById('findResultsText').textContent = `Replaced ${totalReplaced} occurrence${totalReplaced !== 1 ? 's' : ''}`;

    // Re-run search to see if there are any remaining matches
    setTimeout(() => {
        performFind();
    }, 1000);
}
...
/**
 * Handle keydown events in the find input
 */
...
/**
 * Handle keydown events in the replace input
 */
