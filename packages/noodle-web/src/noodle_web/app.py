...
class ActionItem(BaseModel):
    ...
    target_date: Optional[str] = Field(None, max_length=50)
...
class ActionExportRequest(BaseModel):
    """Request body for Actions Excel export."""
...
async def get_actions(
    ...
    from .database import get_db, Action as ActionModel
    ...
    try:
        with get_db() as db:
            ...
            if resource:
                query = query.filter(ActionModel.resource == resource)
            if priority:
                query = query.filter(ActionModel.priority == priority)

            actions = query.order_by(ActionModel.created_at.desc()).all()
            ...
            for action in actions:
                result.append({
                    ...
                    'target_date': action.target_date.isoformat() if action.target_date else '',
...
async def create_action(data: ActionCreateRequest):
    ...
    try:
        with get_db() as db:
            ...
            if data.target_date:
                try:
                    target_date = dt.strptime(data.target_date, '%Y-%m-%d').date()
                except ValueError:
...
async def update_action(action_id: int, data: ActionUpdateRequest):
    ...
    try:
        with get_db() as db:
            ...
            if data.target_date is not None:
                if data.target_date:
                    try:
                        action.target_date = dt.strptime(data.target_date, '%Y-%m-%d').date()
                    except ValueError:
                        raise HTTPException(status_code=400, detail="Invalid date format. Use YYYY-MM-DD")
                else:
                    action.target_date = None
            if data.resource is not None:
                action.resource = data.resource
        ...
        logger.error(f"Error updating action: {str(e)}", exc_info=True)
...
async def delete_action(action_id: int):
    ...
    try:
        ...
        logger.error(f"Error deleting action: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to delete action: {str(e)}")


@app.post("/api/actions/export-excel")
async def export_actions_excel(data: ActionExportRequest):
    """Export action items to an Excel file."""
    from openpyxl import Workbook
    from openpyxl.styles import Font, Alignment, PatternFill
    from openpyxl.utils import get_column_letter
    ...
    for row_idx, item in enumerate(data.items, 2):
        ...
        ws.cell(row=row_idx, column=4, value=item.owner)

        status_cell = ws.cell(row=row_idx, column=5, value=item.status.capitalize())
        if item.status == 'closed':
            status_cell.fill = PatternFill(start_color="D3F9D8", end_color="D3F9D8", fill_type="solid")

        priority_cell = ws.cell(row=row_idx, column=6, value=item.priority.capitalize())
        if item.priority == 'high':
            priority_cell.fill = PatternFill(start_color="FFE0E0", end_color="FFE0E0", fill_type="solid")
        elif item.priority == 'medium':
            priority_cell.fill = PatternFill(start_color="FFF3BF", end_color="FFF3BF", fill_type="solid")

        ws.cell(row=row_idx, column=7, value=item.target_date)
        ws.cell(row=row_idx, column=8, value=item.resource)

    column_widths = [6, 25, 35, 15, 12, 12, 15, 15]
    for col, width in enumerate(column_widths, 1):
        ws.column_dimensions[get_column_letter(col)].width = width
    ...
    try:
        ...
        return Response(
            ...
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={
                "Content-Disposition": f'attachment; filename="{project_name}-actions.xlsx"'
            }
        )
    finally:
        if tmp_path and os.path.exists(tmp_path):
...
@app.post("/api/actions/import-excel")
async def import_actions_excel(file: UploadFile = File(...)):
    """Import action items from an Excel file."""
    from openpyxl import load_workbook
    ...
    try:
        ...
        field_names = {
            ...
            'target date': 'target_date', 'resource': 'resource'
        }
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            item_status = str(get_cell('status', 'open')).lower().strip()
            item_priority = str(get_cell('priority', 'medium')).lower().strip()

            item = {
                'id': int(get_cell('id', len(items) + 1)) if get_cell('id') else None,
                'title': str(get_cell('title', '')),
                'description': str(get_cell('description', '')),
                'owner': str(get_cell('owner', '')),
                'status': item_status if item_status in valid_statuses else 'open',
                'priority': item_priority if item_priority in valid_priorities else 'medium',
                'target_date': str(get_cell('target_date', '')),
                'resource': str(get_cell('resource', '')),
            }
            items.append(item)
        ...
        logger.error(f"Error importing Actions Excel: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=400,
            detail=f"Failed to parse Excel file: {str(e)}"
        )
