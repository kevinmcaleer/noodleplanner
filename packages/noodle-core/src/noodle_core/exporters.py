...
def yaml_to_markdown_table(yaml_path, terminal_width=80):
    """Render the YAML plan file at *yaml_path* as a markdown table.

    The file's contents are both the plan and its front matter (holidays,
    resources), exactly as text_to_markdown_table takes them -- this used
    to be a ~240-line copy of that function's body, differing only in
    formatting.
    """
    with open(yaml_path, encoding="utf-8") as f:
        original_text = f.read()
    return text_to_markdown_table(
        original_text, is_yaml=True, terminal_width=terminal_width, original_text=original_text,
    )
