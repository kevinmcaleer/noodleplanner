...
but **they are changed in Penpot, not in that file.** The token block at its
top, between the `BEGIN PENPOT TOKENS` / `END PENPOT TOKENS` comments, is
generated by `npm run design:tokens` from `docs/design/tokens/*.json`, which is
Penpot's token export. Never hand-edit that block or override one of its tokens
elsewhere in the file. To change a token, change it in Penpot, export the sets
over `docs/design/tokens/`, and run `npm run design:tokens`. The only tokens
edited in the CSS itself are the code-owned ones (the `*-gradient` and
`--np-anim-*` tokens, listed in `CODE_OWNED` in `scripts/design-tokens.mjs`),
which Penpot has no type for.

Before pushing a change that touches CSS, run the design gates:
...
npm run design:tokens -- --check  # the generated tokens match the Penpot export
```

All three are in CI as the gating `design` job. The linter is a ratchet against
